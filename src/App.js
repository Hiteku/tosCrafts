    ...
    <div className="craft-info" ref={craftInfoRef} style={style}>
      ...
      {(() => {
        ...
        .replace(/\[(.*?)\((\d+)\)\]/g, (match, name, id) => {
          const imgUrl = `https://d1h5mn9kk900cf.cloudfront.net/toswebsites/gallery/icons/${String(id).padStart(4, '0')}.jpg`;
      ...
      {(craft.suitable) && (<><hr/><strong>指定角色</strong><br/>{(() => {
        var text = craft.suitable.replace(/\[(.*?)\((\d+)\)\]/g, (match, name, id) => {
          var imgUrl = `https://d1h5mn9kk900cf.cloudfront.net/toswebsites/gallery/icons/${String(id).padStart(4, '0')}.jpg`;
...
function App() {
  ...
  const commonKeywords = ['CD-2', 'CD-3', '獵殺', '人攻', '回血', '喚靈', '熔爐', '黑金', '通行證'];
